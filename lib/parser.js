...
export function extractSignature(text, maxLines = 40) {
  ...
  for (let i = lines.length - 1; i > Math.max(0, lines.length - 120); i--) {
    ...
    if (/^(_{2,}|-{2,})|mit (freundlichen?|besten?|herzlichen?|lieben?|kollegialen?|vielen?)|with (kind|best|warm) regards|best regards|kind regards|sincerely|freundlich|herzlich|viele gr|gruss|greets|gruesse|^mfg\s*$|^lg[,.]?\s*$|^vg\s*$|^ciao\s*$|^tschuess\s*$|^auf wiedersehen/i.test(t)) {
